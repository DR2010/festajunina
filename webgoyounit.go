/* webgoyounit main web application program for the younit website
-----------------------------------------------
.../src/festajuninaweb/webgoyounit.go
-----------------------------------------------
02-Sep-2023

It will be a single web router, multiple apis
- webgoyounit - website running on port :80

CTRL+B - show left
CTRL+J - show terminal
*/
package main

import (
	"database/sql"
	"webgoyounit/areas/activitieshandler"
	admshandler "webgoyounit/areas/admshandler"
	"webgoyounit/areas/coinspothandler"
	...
	"log"
	"net/http"
	cachehandler "webgoyounit/areas/cachehandler"
	"webgoyounit/areas/commonstruct"
	"webgoyounit/areas/ordershandler"
	"webgoyounit/areas/security"

	// The Models are shared by WEB and API
	disheshandler "webgoyounit/areas/disheshandler"
...
func loadreferencedatainpostgresql() {
	variable := helper.Readfileintostruct()
	err = redisclient.Set(sysid+"APIMongoDBLocation", variable.APIMongoDBLocation, 0).Err()
